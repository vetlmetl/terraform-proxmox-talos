...
variable "node_ipv4_cidr" {
  # Used to select each node's primary IP from the QEMU guest-agent report by
  # subnet match, rather than a fragile positional interface index. A VIP in this
  # subnet (e.g. cluster_endpoint) is excluded so it is never mistaken for a node.
  description = "IPv4 CIDR of the network the nodes get their (DHCP) addresses on, e.g. 192.168.88.0/24"
  type        = string

  validation {
    condition     = can(cidrhost(var.node_ipv4_cidr, 0))
    error_message = "node_ipv4_cidr must be a valid IPv4 CIDR, e.g. 192.168.88.0/24."
  }
}
